...
const MainVideo: React.FC = () => {
  ...
  const emailInputProgress = Math.max(0, Math.min(Math.floor(interpolate(frame, [40, 80], [0, 20], { extrapolateRight: 'clamp' })), 20));
  const passwordInputProgress = Math.max(0, Math.min(Math.floor(interpolate(frame, [85, 115], [0, 8], { extrapolateRight: 'clamp' })), 8));
  ...
  const displayEmailText = emailText.substring(0, emailInputProgress);
      ...
      >
        ...
        >
          ...
          <div style={{ marginBottom: '50px' }}>
            ...
            >
              {passwordInputProgress > 0 && '•'.repeat(passwordInputProgress)}
